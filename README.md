...
* (12/19/2018) When creating and deleting a custom domain with a empty basepath and no code changes to deploy, the basePathMapping is not being set. This seems to be an issue with CloudFormation resources not applying when lambda code is not changed. The workaround is to only create/delete a custom domain in conjunction with running `sls remove` 
