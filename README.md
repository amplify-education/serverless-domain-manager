...
| securityPolicy                          | `TLS_1_2`                                                                           | The security policy to apply to the custom domain name. Any value accepted by AWS API Gateway is valid (e.g. `TLS_1_0`, `TLS_1_2`, `SecurityPolicy_TLS13_2025_EDGE`). When using enhanced security policies (those starting with `SecurityPolicy_`), `endpointAccessMode` is automatically set to `STRICT` unless explicitly configured. |
| endpointAccessMode                      | `undefined`                                                                         | The endpoint access mode for the custom domain. Can be `STRICT` or `BASIC`. Required when using enhanced security policies (e.g. `SecurityPolicy_TLS13_1_3_2025_09`). If not specified, defaults to `STRICT` for enhanced security policies. |
